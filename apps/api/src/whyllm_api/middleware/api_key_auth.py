...
Flow (p99 optimised):
  1. Extract raw key from X-whyllm-Key header.
...
async def verify_api_key(
    request: Request,
    x_whyllm_key: Annotated[Optional[str], Header(alias="X-whyllm-Key")] = None,
) -> KeyContext:
    """FastAPI dependency — resolves and returns a KeyContext or raises HTTP 401."""
    if not x_whyllm_key:
        raise HTTPException(
            status_code=401,
            detail={"error": "missing_api_key", "message": "X-whyllm-Key header is required"},
        )

    # 1. Fast path: Redis cache hit
    ctx = await _lookup_in_redis(x_whyllm_key)
    ...
    if ctx is None:
        ...
        asyncio.create_task(_cache_in_redis(x_whyllm_key, ctx))
