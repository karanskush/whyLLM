      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KpiCard
            title="Error Rate"
            value={
              summaryLoading ? "…" : summary ? `${(summary.error_rate * 100).toFixed(1)}%` : "—"
            }
            icon={AlertTriangle}
            iconColor={summary && summary.error_rate > 0.05 ? "bg-red-500/80" : "bg-amber-500/80"}
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-sm font-semibold text-zinc-200">Cost over time</h2>
          </div>
          {timeseries ? (
            <CostLineChart series={timeseries.series} granularity={timeseries.granularity} />
          ) : (
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-xs text-zinc-500">Last 10 events</span>
          </div>
          {liveEvents.length === 0 ? (
            ...
            <div className="space-y-1">
              {liveEvents.map((ev, i) => (
                ...
                >
                  ...
                  <span className="text-xs text-zinc-500">{formatLatency(ev.latency_ms)}</span>
                  <span className="text-xs text-zinc-400 font-mono">{formatCost(ev.cost_usd)}</span>
                </div>
              ))}
            </div>
          )}
