...
from llmdawg_api.routes.spans import _check_project_access
from llmdawg_api.schemas.settings import (
    AlertCreate,
    AlertResponse,
    ApiKeyCreate,
...
def _generate_api_key(environment: str) -> tuple[str, str, str]:
    ...
    raw_key = f"ld-{env_short}_{token}"
...
def _fire_test_notification(channel: dict) -> None:
    ...
    try:
        ...
        if ch_type == "webhook":
            _httpx.post(target, json={"type": "test", "message": "LLMDawg alert test"}, timeout=5.0)
        elif ch_type == "slack":
            _httpx.post(target, json={"text": "LLMDawg alert test ping"}, timeout=5.0)
