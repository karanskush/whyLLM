...
Covers:
- Header stripping (remove LLMDawg-internal + hop-by-hop headers)
...
# LLMDawg-specific request headers that must not reach upstream providers
_LLMDAWG_PREFIXES = ("x-llmdawg-",)
...
def strip_proxy_headers(headers: Any) -> dict[str, str]:
    ...
    for k, v in headers.items():
        ...
        if kl in _STRIP_HEADERS:
            continue
        if any(kl.startswith(prefix) for prefix in _LLMDAWG_PREFIXES):
            continue
        result[k] = v
    return result
