"""LLMDawg CLI — verify connectivity and send test spans."""
...
@click.option(
    "--api-key",
    envvar="LLMDAWG_API_KEY",
    default=None,
    help="LLMDawg API key (or set LLMDAWG_API_KEY env var).",
)
@click.option(
    "--base-url",
    envvar="LLMDAWG_BASE_URL",
    default="http://localhost:8000",
    show_default=True,
    help="LLMDawg API base URL.",
)
@click.pass_context
def cli(ctx: click.Context, api_key: str | None, base_url: str) -> None:
    """LLMDawg command-line tools."""
    ctx.ensure_object(dict)
    ctx.obj["client"] = LLMDawgClient(api_key=api_key, base_url=base_url)


@cli.command()
@click.pass_context
def verify(ctx: click.Context) -> None:
    """Check connectivity to the LLMDawg API."""
    client: LLMDawgClient = ctx.obj["client"]
    click.echo(f"Connecting to {client.base_url} …")
    if client.verify():
        click.secho("OK — LLMDawg API is reachable.", fg="green")
    else:
        click.secho(
            "FAIL — could not reach LLMDawg API. "
...
def test_ingest(ctx: click.Context, model: str, provider: str) -> None:
    ...
    span = {
        ...
        "request": {
            "model": model,
            "messages": [{"role": "user", "content": "llmdawg test-ingest ping"}],
        },
        "response": {
            "content": "pong",
            "finish_reason": "stop",
