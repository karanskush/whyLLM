...
_client: LLMDawgClient | None = None
...
def init(
    ...
    """Initialize the global LLMDawg tracer.

    Call once at application startup before making any LLM calls.

    Args:
        api_key: Project API key (ld-...). Falls back to LLMDAWG_API_KEY env var.
    ...
    Returns:
        The initialized LLMDawgClient for advanced use cases.
    """
    global _client, _sender
    _client = LLMDawgClient(api_key=api_key, base_url=base_url)
...
def wrap(client: Any) -> Any:
    ...
    if _sender is None:
        raise RuntimeError(
            "llmdawg.init() must be called before llmdawg.wrap(). "
            "Example: llmdawg.init(api_key='ld-...')"
        )

    module = type(client).__module__
    if "openai" in module:
        from llmdawg.wrappers.openai_wrapper import wrap_openai
        return wrap_openai(client, _sender)
    if "anthropic" in module:
        from llmdawg.wrappers.anthropic_wrapper import wrap_anthropic
